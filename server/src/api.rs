...
impl MutationRoot {
    ...
    async fn update_username(&self, _ctx: &Context<'_>, id: i32, username: String) -> Result<User> {
        Ok(update_username(id, username))
    }
    async fn update_email(&self, _ctx: &Context<'_>, id: i32, email: String) -> Result<User> {
        Ok(update_email(id, email))
    }
    async fn update_password(&self, _ctx: &Context<'_>, id: i32, password: String) -> Result<User> {
        Ok(update_password(id, password))
    }
    async fn delete_user(&self, _ctx: &Context<'_>, id: i32) -> Result<User> {
        Ok(delete_user(id))
    }
    async fn create_lamp(
        &self,
        _ctx: &Context<'_>,
        user_id: Option<i32>,
        name: String,
    ...
    async fn set_lamp_is_on(&self, _ctx: &Context<'_>, id: i32, is_on: bool) -> Result<Lamp> {
        Ok(toggle_on(id, is_on))
    }
    async fn update_lamp_name(&self, _ctx: &Context<'_>, id: i32, name: String) -> Result<Lamp> {
        Ok(update_name(id, name))
    }
    async fn update_lamp_description(
        &self,
        _ctx: &Context<'_>,
        id: i32,
        description: String,
    ...
    async fn delete_lamp(&self, _ctx: &Context<'_>, id: i32) -> Result<Lamp> {
        Ok(delete_lamp(id))
    }
}
